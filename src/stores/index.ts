    ...
    store.$onAction(({ name, after, onError }: any) => {
      ...
      after(() => {
        ...
        if (import.meta.env.DEV && duration > 100) {
          console.warn(
            `[Store Performance] Slow action detected: ${storeId}.${name} took ${duration.toFixed(2)}ms`
          )
        }
      })
    })
...
function createErrorHandlingPlugin() {
  ...
  return ({ store }: any) => {
    ...
    store.addGlobalErrorHandler = (
      handler: (error: any, store: string, action: string) => void
    ) => {
...
export class StoreManager {
  ...
  static enableDevMode() {
    if (typeof window !== 'undefined') {
      ...
      console.log(
        '[Store Manager] Dev mode enabled. Use __STORE_MANAGER__ to access store utilities.'
      )
    }
  }
}
