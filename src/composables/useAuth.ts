...
export function useStoreAdmin() {
  ...
  const isStoreAdmin = computed(() => 
    isPlatformAdmin.value || hasRole('store-admin')
  );

  const canManageStore = computed(() => 
    isPlatformAdmin.value || hasAnyRole(['store-admin'])
  );
...
export function useAuthLoading() {
  ...
  const isAuthReady = computed(() => 
    !isInitializing.value && status.value !== 'loading'
  );
  ...
  const isAuthenticating = computed(() => 
    status.value === 'loading' && !isInitializing.value
  );
